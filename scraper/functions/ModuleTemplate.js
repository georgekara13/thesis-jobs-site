...
module.exports =
{
    results_page: async function(ad_urls,driver)
    {
        ...
        try
        {
            ...
            await driver.findElement(webdriver.By.className('')).sendKeys(webdriver.Keys.ENTER);
        }
        catch(err)
        {
        ...
        //FETCH AD URLS
        var fetched_urls = await driver.findElements(webdriver.By.className(""));
    ...
    aad_page: async function(driver,ad_url,ad_fields,conf_filter)
    {
      ...
      currentdate     = currentdate.getDate() + "/" +
        ...
        */
        ...
        for(var i = 0; i<3; i++)
        {
            try
            {
                var ad_title        = await driver.findElement(webdriver.By.xpath('')).getText().then(function(title){ad_fields["title"] = title;});
                ...
                var ad_location     = await driver.findElement(webdriver.By.className("")).getText().then(function(location){ad_fields["location"] = location;});
                var ad_id           = await driver.findElement(webdriver.By.xpath("")).getText().then(function(id){id = id.replace(/^.*?:\s*/, "");ad_fields["id"] = id;return id;});
                var ad_company      = await driver.findElement(webdriver.By.xpath('')).getText().then(function(company){ad_fields["company"] = company;});
                var ad_date         = await driver.findElement(webdriver.By.xpath('')).getText().then(function(date){ad_fields["date"] = date;});
                var ad_type         = await driver.findElement(webdriver.By.xpath('')).getText().then(function(type){ad_fields["type"] = type;});
